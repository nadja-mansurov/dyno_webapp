...
import { PlayerActions, SelectionActions } from '@/app/actions/action-types';
...
export class NglIndexComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private toggleSelected(isAll?: 'show'|'hide') {
    ...
    if (this.isSelected) {
      ...
      const range = this.parserService.getShowingIndecies([this.globalMin, this.globalMax],
          isAll, this.globalMin, this.globalMax);
      this.dynophoreShapes = this.parserService.dynophoreDrawingByVisible(this.dynophore, range);
      this.showDynophore();
  ...
  private stageClicked(clicked: any) {
    ...
    if (clicked?.picker?.shape?.name) {
      ...
      this.store.dispatch(SelectionActions.setSelected({
        selected: this.parserService.getFeatureCloudInfo(clicked?.picker?.shape?.name) }));
